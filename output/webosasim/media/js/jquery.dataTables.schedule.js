...
MyNameSpace.AddRow=function(id,name,start,service)
{
	/*create a new process and add it to processes Array and dataTable. then go to last page to see new row*/
	var proc=new MyNameSpace.process(name,start,service);
	if(id!=-1)
	{
		proc.id=id;
	}
	...
	MyNameSpace.oTable.fnAddData( [proc.id,proc.name,proc.entranceTime,proc.serviceTime,'<a href="javascript:void(0);" onclick="fnClickDeleteRow(this);">حذف</a>'] );
	...
	*/
	...
	var EditableCallback = function(v, s) {
		...
		if(column==0||(column==1&&v.length>20)||(column==2&&isNaN(v))||(column==2&&v.length>2)||(column==3&&isNaN(v))||(column==3&&v.length>2))
		{
			editValFailure=true;
		}
		else
	...
	$('td[class!="readonly"]', MyNameSpace.oTable.fnGetNodes()).editable(EditableCallback, {
		"callback": function( sValue, y ) {
			...
			if(editValFailure==false)
			{
				...
				MyNameSpace.UpdateProcess(aData[0], aData[1], aData[2], aData[3]);
			}
			...
			else
			{
				for(var i=0;parseInt(i,10)<parseInt(MyNameSpace.numberOfProcesses,10);i++)
				{
					if(MyNameSpace.processes[i].id==aData[0])
					{
						...
						v[3]=MyNameSpace.processes[i].serviceTime;
...
function fnClickAddRow() {
	...
	var randomService=Math.floor(Math.random()*20)+1;
	var name="proc"+(MyNameSpace.numberOfProcesses+1);
	MyNameSpace.AddRow(-1,name,randomStart,randomService);
}
